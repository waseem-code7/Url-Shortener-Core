        ...
        if registry_name == SHORTENER_URL_SERVICE and registry_name not in ServiceRegistry.__cache:
            with ServiceRegistry.__lock:
                if registry_name not in ServiceRegistry.__cache:
                    ...
                    shortener_service = Shortener(ShortenerFactory.get_strategy(os.getenv("SHORTENER_STRATEGY")))
            ...
            with ServiceRegistry.__lock:
                if registry_name not in ServiceRegistry.__cache:
                    user_repository = UserRepository("USER_DATA")
                    ServiceRegistry.__cache[AUTH_SERVICE] = AuthService(user_repository=user_repository)
